...
# The ATOM listens at servo ID 7; servos 1-6 ignore ID 7 traffic.
# ---------------------------------------------------------------------------

ATOM_ID            = 7
ATOM_ADDR_PING     = 0x00
ATOM_ADDR_SET_COLOR = 0x01
ATOM_ADDR_SET_PIXEL = 0x02
...
class ServoController:
    ...
    # ---- ATOM commands (Feetech ID 7) ----

    def _atom_write(self, address: int, data: bytes = b"") -> bytes | None:
        resp = self._write_raw(ATOM_ID, address, data)
        return parse_status_params(resp)

    def atom_ping(self) -> bool:
        with self._lock:
            return self._atom_write(ATOM_ADDR_PING) is not None

    def atom_set_color(self, r: int, g: int, b: int):
        with self._lock:
            self._atom_write(ATOM_ADDR_SET_COLOR, bytes([r, g, b]))

    def atom_set_pixel(self, x: int, y: int, r: int, g: int, b: int):
        with self._lock:
            self._atom_write(ATOM_ADDR_SET_PIXEL, bytes([x, y, r, g, b]))
            ...
            while b"\n" in buf:
                ...
                # ---- ATOM_COLOR <r> <g> <b> ----
                    ...
                    if len(parts) < 4:
                        ...
                        continue
                    ctrl.atom_set_color(int(parts[1]), int(parts[2]), int(parts[3]))
                    reply("OK")
                ...
                # ---- ATOM_PIXEL <x> <y> <r> <g> <b> ----
                    ...
                    if len(parts) < 6:
                        ...
                        continue
                    ctrl.atom_set_pixel(int(parts[1]), int(parts[2]),
